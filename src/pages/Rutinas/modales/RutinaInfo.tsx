...
const EjercicioInfoModal = ({ exercise, onClose }: any) => (
  /* Aplicamos el mismo desplazamiento para que el sub-modal también esté centrado */
  <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 lg:left-72 xl:right-80 animate-in fade-in duration-300">
    ...
    <div className="relative bg-[#161925] border border-white/10 w-full max-w-sm rounded-[2rem] p-6 shadow-2xl">
      ...
      <Text size="lg" weight="black" variant="gradient" className="uppercase mb-4 text-center italic">
        {exercise.nombre}
      </Text>

      <div className="aspect-video bg-black/40 rounded-xl mb-4 overflow-hidden border border-white/5 shadow-inner">
      ...
      <div className="bg-white/5 p-3 rounded-xl border border-white/5 mb-4">
        <Text size="xs" className="text-gray-400 leading-tight italic line-clamp-3">
          {exercise.descripcion || "Enfoque en técnica y control absoluto para maximizar el reclutamiento muscular."}
        </Text>
      </div>
...
// --- MODAL PRINCIPAL DE DETALLE DE RUTINA ---
const RutinaDetalle = ({ rutina, onClose }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 lg:left-72 xl:right-80 transition-all duration-300">
      <div 
        className="absolute inset-0 bg-[#0f111a]/95 backdrop-blur-xl animate-in fade-in duration-500" 
        onClick={onClose} 
      />
      <div className="relative bg-[#161925] border border-white/10 w-full max-w-lg max-h-[80vh] rounded-[2.5rem] overflow-hidden flex flex-col shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="relative h-32 flex-shrink-0 bg-gradient-to-b from-purple-600/10 to-transparent flex items-end px-8 pb-4">
          <button 
            onClick={onClose} 
            className="absolute top-6 right-6 z-20 bg-black/40 p-2 rounded-full text-white hover:bg-purple-600 transition-all shadow-lg"
          >
            <FaTimes size={14} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-8 pt-2 no-scrollbar">
          <div className="mb-6">
            <p className="text-gray-500 text-[11px] leading-relaxed border-l-2 border-purple-500/30 pl-4 py-1 italic">
              {rutina.descripcion || "Plan de entrenamiento optimizado para el desarrollo de fuerza y masa muscular."}
            </p>
          </div>
